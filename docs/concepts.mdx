...
Each layer only depends on the one below it. Areas don't have their own storage —
they attach a wiki page. Area agents aren't a separate runtime — they're
...
An area is a domain — apartment hunt, health, a side project. It may attach one
...
That cap is the whole point. Every run re-decides the area's asks against stable
keys, so you get a current shortlist instead of a growing backlog. Home shows
each area's current ask gathered into one focus set. **A quiet area is a good
outcome, not a gap.**
