...
Every automation runs under a **tool scope**, and you choose between two:
`read_only` (the default — the runs read but never write or execute) and `all`.
Pick `all` only when the prompt genuinely has to act. `auto_approve` skips
approval for tools the scope already grants; it never widens the scope. See
[tool scoping](/guides/tools#per-automation-tool-scoping).

## Area agents

[Area agents](/guides/areas) are channel automations seeded one per delegated area. Their explicit tool scope includes the terminal `area_submit_report` action that validates and commits work, evidence, asks, and the next-check cadence.
...
By default, mutating or external actions still require approval. Leave an
automation `read_only` unless it must act; toggle **auto-approve** only when the
actions its scope already grants must run headlessly without per-call approval.
