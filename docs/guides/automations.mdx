...
Automations let arden act on its own — daily summaries, inbox monitoring, reminders, or any task you'd otherwise forget to do manually.
...
An automation can own a durable **channel session**. Its runs land in a real session whose transcript you can open, read, and reply to, and where approvals surface in-session. This is how iteration-style monitors and [area agents](/guides/areas) keep run-to-run memory instead of starting fresh each fire.

## Tool scoping

An automation can carry a **tool scope**: an allowlist of tool-name patterns (`*`, an exact name, or `prefix*`) that bounds what its runs may touch, applied as a hard outer gate. An area agent uses this to stay observe-only. See [tool scoping](/guides/tools#per-automation-tool-scoping).
...
[Area agents](/guides/areas) are ordinary automations, seeded one per area, with a channel, an observe-only tool scope, and the `area_ask` output schema. They appear under **Area agents** in the Active tab and can be edited, paused, rescheduled, or run like any other automation.

## Builtin automations

Arden ships with system automations that run automatically, such as memory consolidation, synthesis, and the area suggester. They appear in the **System** tab. What they do is fixed, but their schedule is yours: adjust the cadence, pause, or run now.
