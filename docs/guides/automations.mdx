...
[Area agents](/guides/areas) are channel automations seeded one per delegated area. Their explicit tool scope includes one terminal `submit_area_report` action that validates and commits work, evidence, asks, and the next-check cadence inside the normal agent loop.

## Builtin automations

Arden ships with Memory Maintenance, Memory Retention, Memory Synthesis, Wiki Maintenance, and optional Memory Dream. They appear in the **System** tab. Their responsibilities are fixed; you can pause them, adjust their cadence, or run them now.
