---
title: "Memory"
description: "How arden remembers, maintains, and recalls what matters."
---

## Overview

Arden keeps long-term memory as one flat pool of small, self-contained
**records**. Each record is a single durable statement, typed by what it does:

- **directive** — a rule or procedure that should steer how arden behaves.
...
Records with a shared subject are periodically **synthesized** into readable
**topic pages** — one markdown file per person, project, or life domain, with
prose, open loops, and wikilinks to related pages. The topic pages are what the
[Slices](/guides/slices) surface is built on: a slice *is* a topic page, watched
by a standing agent.
...
"what concrete, durable thing should arden know?"
...
For every prompt, arden combines two layers:
...
![Memory topic pages in the desktop app](../images/memory-patterns.png)

- **Browse** the generated topic pages (the **Files** view) and the underlying
  records (the **Records** view).
- **Search** by keyword or meaning, inspect scope, source, and lifecycle.
