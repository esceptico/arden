...
4. `.skills/` — ntrp project-level skills (per-directory)
5. `~/.ntrp/skills/` — global user skills
...
Restart the server after creating a skill manually so the registry rescans builtin skills, `agent/skills/`, `.agents/skills/`, `.skills/`, `~/.ntrp/skills/`, and `~/.agents/skills/`. Skills installed through the API are reloaded automatically.
