...
This is how [slice agents](/guides/slices) stay observe-only. They get the read and memory-write tools they need and nothing that acts on the outside world, regardless of other flags. A scope of `null` (the default) is unrestricted.
...
Automations reference a named schema (`output_schema`), resolved on the server. This is the mechanism behind a slice agent's one-ask nomination: rather than parsing free-form text for an ask, the run returns `{ ask: {...} | null }` as a guaranteed-shaped object.

## User-defined tools

Create custom tools in `~/.ntrp/tools/`. Each file exports a `tools` mapping. Inputs must be bounded, failures typed, and stable results sourced:
