...
| `list_wiki_changes` | List pages changed since a timestamp, with actor and reason. | – |
...
| `list_automation_runs` | List automation executions since a timestamp | – |
...
`all_tools` on `create_automation`. Nothing else is selectable — an automation
...
`submit_area_report`. Invalid or stale reports return a tool error inside the
