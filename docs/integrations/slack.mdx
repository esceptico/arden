...
arden currently supports Slack through workspace tokens. A polished Slack OAuth install flow can be added later, but today the desktop setup assistant makes the token path explicit and verifies the token before saving.
...
Paste the token into the assistant and click verify. arden calls Slack `auth.test` and shows the workspace/user details if the token works.

### Save the connection

After verification, save the token from the Integrations card. arden reloads integration clients automatically.
