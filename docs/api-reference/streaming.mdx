...
Arden uses a persistent SSE connection per session. Connect to `GET /chat/events/{session_id}` to receive all events (agent responses, tool calls, background task results). Messages are sent via `POST /chat/message` as fire-and-forget — the response arrives on the SSE stream.
...
Tool arguments. ARDEN currently emits arguments atomically as a JSON string.
