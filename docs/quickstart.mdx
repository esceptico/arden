...
<Note>
`uv tool install arden` installs the backend CLI. The Electron desktop app currently runs from a source checkout.
</Note>
...
<Warning>Copy this key — it's only shown once. Use `arden-server serve --reset-key` to generate a new one.</Warning>
