...
/** Horizontal strip of every area as a tonal chip: a live dot marks areas
 *  with active asks or a running agent; quiet areas sit at 55% opacity so
 *  the live ones read as "where attention currently is."
 *
 *  Suggested areas (the daily suggester's picks from unpromoted topic
 *  pages) render as ghost chips at the end — dashed, with a plus; click
 *  promotes the page into a real area, the ✕ dismisses it permanently.
...
export function AreasStrip({
  ...
  if (areas.length === 0 && suggested.length === 0) return null;

  return (
    <div className="grid gap-2">
      <span className="text-2xs font-semibold tracking-wide text-faint uppercase">Areas</span>
      <div className="flex flex-wrap gap-1.5">
        {areas.map((area) => (
          <button
            ...
            className={clsx(
              ...
              !area.live && "opacity-50 hover:opacity-100",
            )}
          >
        ...
        {suggested.map((s) => (
          ...
          >
            <button
              type="button"
              onClick={() => void promoteSuggestedArea(s.title, s.page_path)}
            ...
            <button
              type="button"
              onClick={() => void dismissAreaSuggestion(s.key)}
