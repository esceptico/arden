...
test("known tools get corpus-specific labels + category icons", () => {
  expect(operationLabel(item({ kind: "file_read" }))).toMatchObject({ verb: "Read", iconKey: "file" });
  ...
  expect(operationLabel(item({ kind: "email_search" }))).toMatchObject({ verb: "Searched email", iconKey: "mail" });
...
test("the corpus, not just the action, is in the label (search tools are distinguishable)", () => {
  const verbs = ["email_search", "slack_search", "web_search", "session_search_transcripts", "fact_search"].map(
...
test("unknown tools fall back to a humanized display_name + a category/dot icon", () => {
  ...
  expect(operationLabel(item({ kind: "automation_list", displayName: "ListAutomations" })).verb).toBe(
    "List Automations",
  );
...
test("detail comes from args, preferring path-like keys; partial JSON is safe", () => {
  expect(operationLabel(item({ kind: "file_read", args: '{"path":"arden/core/agent.py"}' })).detail).toBe(
    "arden/core/agent.py",
  );
  expect(operationLabel(item({ kind: "file_read", args: '{"path":' })).detail).toBeNull();
  expect(operationLabel(item({ kind: "file_read" })).detail).toBeNull();
  expect(operationLabel(item({ kind: "tool_search", args: '{"tools":["slack_search","email_search"]}' })).detail).toBe(
    "slack_search, email_search",
  );
});

test("groupSummary pluralizes with the tool's noun, else falls back to a count", () => {
  expect(groupSummary([item({ kind: "file_read" }), item({ kind: "file_read" }), item({ kind: "file_read" })])).toMatchObject({
    verb: "Read 3 files",
    iconKey: "file",
  });
  ...
  expect(groupSummary([item({ kind: "email_search" }), item({ kind: "email_search" })]).verb).toBe("Searched email · 2");
  // No noun → "{label} · {n}".
...
test("without backend hints, the client registry still drives icon + noun (history reload)", () => {
  expect(operationLabel(item({ kind: "file_read" }))).toMatchObject({ iconKey: "file", noun: "file" });
...
test("a real tool title is behavior data, never display metadata", () => {
  const r = operationLabel(
    item({ kind: "email_search", args: '{"title":"Quarterly report","query":"acme invoice"}' }),
  );
...
test("group summaries stay stable per-kind, ignoring per-call model titles", () => {
  const rows = [
    item({ kind: "file_read", displayTitle: "Reading the spec", args: '{"path":"a.ts"}' }),
    item({ kind: "file_read", displayTitle: "Reading the impl", args: '{"path":"b.ts"}' }),
    item({ kind: "file_read", displayTitle: "Reading the test", args: '{"path":"c.ts"}' }),
  ];
