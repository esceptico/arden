...
Create a normal automation whose results live under `automations/`. The automation gets its own dedicated channel automatically. Do not attach the current chat; `loop_create` is the separate current-chat workflow.
...
2. Call `wiki_list_pages` with the root directory (`directory=""`). A `not_found`
...
5. Call `wiki_create_page` when absent with its exact managed path, title,
   aliases, and body; otherwise call `wiki_edit_page` with the exact path and body.
...
Call `automation_create` once. Set:

- `auto_approve=true`;
- `all_tools=true` — the prompt must write wiki pages, and runs are read-only without it;
...
The normal read-only wiki tools are available in every run; `all_tools` only unlocks writes.

If creation has an ambiguous result, first call `automation_list`. Retry with the exact same key only when that producer is absent. Never make a `-2`, date, or versioned key to work around a retry. When the producer already exists, call `automation_update` instead of creating another one. An explicit deletion releases the global key, so recreating the same producer uses the same key.
