...
def test_known_tools_map_to_icon_and_noun():
    assert tool_presentation("file_read", "_system") == ("file", "file")
    assert tool_presentation("email_read", "gmail") == ("mail", "email")
...
def test_search_tools_carry_no_grouping_noun():
    ...
    for name, source in [
        ...
        ("session_search_transcripts", "_sessions"),
        ("fact_search", "_facts"),
        ("file_search_text", "_system"),
        ("tool_search", "_system"),
        ("drive_search", "google_drive"),
    ]:
