...
class Scheduler:
    ...
    async def _release_untracked_running(self) -> None:
        ...
        for automation in await self.store.list_running():
            ...
            if dispatched_at is not None:
                ...
                await self.store.fail_latest_running_run(automation.task_id, now, "run never reported back")
    ...
    async def _run_session_bound(self, automation: Automation, context: str | dict | None = None) -> str | None:
        ...
        result: str | DetachedRun | None = DetachedRun(dispatched) if automation.read_history else dispatched
