...
def initialize_empty_vault(root: Path) -> None:
    ...
    (root / ".arden").mkdir(exist_ok=True)
...
def _entries(path: Path, root: Path, malformed: list[str]) -> list[LedgerEntry]:
    ...
    if first is None or RECORDS_VERSION_RE.fullmatch(rows[first]) is None:
        return []
...
def validate_vault(root: Path) -> VaultHealth:
    ...
    if raw_root.is_symlink():
        ...
        for path in sorted(raw_root.rglob("*.md")):
            ...
            header = RECORDS_VERSION_RE.fullmatch(first)
            if header is None:
                if first:
                    versions.add(1)
                continue
    ...
    interrupted = (
        tuple((Path(".arden/journal") / child.name).as_posix() for child in sorted(journal.iterdir()))
        if journal.is_dir()
        else ()
    )
