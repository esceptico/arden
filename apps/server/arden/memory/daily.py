...
from arden.memory.frontmatter import QuotedStr, dump_frontmatter, parse_frontmatter
...
from arden.memory.models import SourceRef, TimePrecision
from arden.memory.page_events import PageEditEvent
...
def daily_base_rel(local_date: date, revision: str, timezone: str) -> Path:
    return Path(".arden/maintenance/daily-bases") / local_date.isoformat() / f"{_projection_key(revision, timezone)}.md"


def daily_candidate_rel(local_date: date, revision: str, timezone: str) -> Path:
    return (
        Path(".arden/maintenance/daily-candidates")
        / local_date.isoformat()
        / f"{_projection_key(revision, timezone)}.md"
    )
...
class DailyProjector:
    ...
    def render(self, local_date: date, revision: str) -> DailyProjection:
        ...
        if not review_required:
            self._assert_revision(revision)
            self._resources.write_daily_maintenance(daily_base_rel(local_date, revision, self.timezone), generated)
            if current != content:
    ...
    def _groups(self, events: tuple[DailyTimelineEvent, ...]) -> tuple[tuple[DailyGroup, ...], bool]:
    ...
    def _read_base(self, local_date: date, revision: str | None, timezone: str | None) -> bytes | None:
        if revision is None or timezone is None:
            return None
        try:
            return self._resources.read_daily_maintenance(daily_base_rel(local_date, revision, timezone))
        except FileNotFoundError:
            return None
