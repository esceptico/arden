...
_INTERNAL_DIRS = {".arden", ".index", ".maintenance"}
_OBSERVED_STATE = Path(".arden/maintenance/observed-pages.json")
_OBSERVED_BASES = Path(".arden/maintenance/observed-page-bases")
...
Its append-only **timeline** of atomic records lives in `raw/<same-path>.md`:
    ...
    - 2026-06-21 ^a1b2c3d4 [fact] [imp:6] Tim rides a gravel bike.
      <!-- arden:meta {"recorded_at":"...","sequence":1,"time_precision":"day","scope":{"kind":"user"},"sources":[...]} -->
...
class FilePageStore:
    ...
    def _editable_page_bytes(self) -> dict[str, bytes]:
        from arden.memory.artifacts import artifact_is_editable
    ...
    def _read_observed_state(self) -> dict:
        ...
        if (
            ...
            or not isinstance(state.get("engine_writes"), list)
    ...
    def _load_or_initialize_observed_pages(self) -> None:
        ...
        self._write_observed_state({"version": 1, "pages": revisions, "engine_writes": [], "pending_changes": {}})
    ...
    def _page_edit_event_committed(self, event_id: str) -> bool:
        from arden.memory.page_events import parse_page_edit_events
    ...
    def _observed_page_changes(self) -> list[ObservedFileChange]:
        ...
        for path, revision in tuple(pages.items()):
            ...
            state["engine_writes"] = [marker for marker in state["engine_writes"] if marker.get("path") != path]
            state_changed = True
        marker_paths = {
            marker["path"]
        ...
        for path in sorted(set(pages) | set(current) | set(state["pending_changes"]) | marker_paths):
            ...
            for marker in path_markers:
                if marker.get("base_revision") != cursor_revision or marker.get("base_exists") != cursor_exists:
                    if marker.get("base_revision") != (before_revision or self._content_revision(None)) or marker.get(
                        "base_exists"
                    ) != (path in pages):
                        continue
            ...
            if matching_marker is not None:
                before = (
                    self._read_observed_file(_OBSERVED_BASES / before_revision) if before_revision is not None else None
                )
                marker_after = (
    ...
    def _reload_canonical_state(self) -> None:
        ...
        active = (
            ...
            else tuple(line for page in self._pages.values() for line in page.active_lines())
        )
    ...
    def _ledger_entries(self) -> tuple[LedgerEntry, ...]:
        return tuple(line for page in self._pages.values() for line in page.lines if isinstance(line, LedgerEntry))
    ...
    def _reconcile_entity(self, slug: str | None, *, display: str | None = None) -> Path | None:
        ...
        if promoted:
            ...
            # frontmatter a junk drawer: dex.md carrying GitHub/ARDEN/Obsidian) and kept
            # casing duplicates (ARDEN + arden) alive. Reset to the canonical display.
            want = [display]
    ...
    def _write_index(self) -> None:
        ...
        if dailies:
            parts += [
                "## Daily",
                "",
                f"- `daily/` — {len(dailies)} dated logs, newest [[daily/{dailies[-1]}|{dailies[-1]}]]",
                "",
            ]
    ...
    def _write_health(self) -> None:
        ...
        last_synth = f"{current_syntheses}/{len(synthesized)} pages at current revision" if synthesized else None
        ...
        if dream_age is None or dream_age > DREAM_DAYS:
            gaps.append(
                f"- Cross-domain dream hasn't run recently (last: {last_dream or 'never'}) — fewer net-new insights."
            )
        ...
        for path, pg in sorted(self._pages.items()):
            ...
            if path.parent.name == _ENTITIES:
                ...
                if not pg.prose and pg.active_lines():
                    gaps.append(
                        f"- Pending synthesis: `{rel}` has {len(pg.active_lines())} records but no compiled prose yet."
                    )
    ...
    def _ensure_page(self, path: Path, *, title: str | None = None) -> Page:
        ...
        if page is None:
            ...
            page = Page(
                frontmatter=fm,
                records_header=f"<!-- arden:records schema=2 page={rel.as_posix()} -->",
            )
            self._pages[path] = page
        return page
        ...
        *,
        ...
        for entry in additions:
            ...
            if page.records_header is None:
                rel = path.relative_to(self._root).as_posix()
                page.records_header = f"<!-- arden:records schema=2 page={rel} -->"
    ...
    async def supersede_with(
        ...
        *,
        ...
        if found and isinstance(found[1], LedgerEntry):
            ...
            successor = LedgerEntry(
                ...
                meta=LedgerMeta(
                    ...
                    sequence=self._next_sequence(),
                    time_precision=(
                        source_ref.time_precision if source_ref and source_ref.occurred_at else old.meta.time_precision
                    ),
        ...
        old_display = (
            self._pages[found[0]].frontmatter.get("title") if (found and found[0].parent.name == _ENTITIES) else None
        )
        ...
        if isinstance(line, LedgerEntry):
            ...
            successor = LedgerEntry(
                ...
                meta=LedgerMeta(
                    ...
                    sequence=self._next_sequence(),
                    time_precision=(
                        source_ref.time_precision if source_ref and source_ref.occurred_at else line.meta.time_precision
                    ),
                    scope_kind=line.meta.scope_kind,
                    scope_key=line.meta.scope_key,
            ...
            if line.meta.scope_kind != "area":
                for slug in {*line.entity, *updated.entity}:
                    self._reconcile_entity(
                        slug, display=next((label for label in (entity_labels or []) if _slug(label) == slug), None)
                    )
    ...
    async def labels_for(self, record_ids: list[str], *, include_kind: bool = False) -> dict:
        ...
        for rid in record_ids:
            ...
            if include_kind:
                out[rid] = [{"label": l, "kind": "entity"} for l in ents] + [
                    {"label": l, "kind": "meta"} for l in metas
                ]
            else:
    ...
    async def merge(
        ...
        if self._ledger_mode():
            ...
            meta_labels = {label for entry in predecessors for label in self._meta_labels(self._loc[entry.id])}
            recorded_at = now_iso()
            successor = LedgerEntry(
                id=self._new_id(),
            ...
            for path, page in self._pages.items():
                ...
                for line in page.lines:
                    if isinstance(line, LedgerEntry) and old_slug in line.entity:
                        entities = tuple(
                            dict.fromkeys(new_slug if entity == old_slug else entity for entity in line.entity)
                        )
...
if __name__ == "__main__":
    ...
    async def _demo():
        with tempfile.TemporaryDirectory() as d:
            ...
            assert any(ln.entity == "bicycles" for p in store2._pages.values() for ln in p.lines), (
                "entity tag persisted"
            )
            ...
            assert any(
                r["label"] == "Bicycles" and r["count"] == 1 and r["kind"] == "entity"
                for r in await store2.list_labels()
            )
            ...
            await store2.add(
                "Tim's bike has 700c wheels.",
                kind="fact",
                source_ref=SourceRef("curator", ""),
                entity_labels=["Bicycles"],
            )
            ...
            await promoted.supersede_with(
                gravel_id, text="Tim now rides a road bike.", source_ref=SourceRef("curator", "")
            )
            assert bike_page.exists() and len(promoted._pages[bike_page].active_lines()) == 2, (
                "still 2 active -> stays promoted"
            )
            ...
            p1 = await again.add(
                "Acme ships widgets.", kind="fact", source_ref=SourceRef("curator", ""), entity_labels=["Acme"]
            )
            p2 = await again.add(
                "Acme is based in Berlin.", kind="fact", source_ref=SourceRef("curator", ""), entity_labels=["Acme"]
            )
            ...
            assert any(ln.entity == "acme" for ln in again._pages[Path(d) / _ME].active_lines()), (
                "record parked on me.md, tagged"
            )
            ...
            q1 = await again.add(
                "Zeta fact one.", kind="fact", source_ref=SourceRef("curator", ""), entity_labels=["Zeta"]
            )
            q2 = await again.add(
                "Zeta fact two.", kind="fact", source_ref=SourceRef("curator", ""), entity_labels=["Zeta"]
            )
            ...
            les = await again.add(
                "Verify against the running system before reporting status.",
                kind="lesson",
                source_ref=SourceRef("curator", ""),
            )
            ...
            with tempfile.TemporaryDirectory() as d2:
                ...
                assert reopened._pages[Path(d2) / "topics" / "interaction-lab.md"].frontmatter["aliases"] == [
                    "Interaction Lab"
                ]
    ...
    async def _demo_vec():
        ...
        class _Store:
            ...
            async def vector_search(self, emb_bytes, sources=None, limit=10):
                ...
                out = [
                    (rid, float(np.dot(q, vec)))
                    for rid, (s, _sid, vec, _m, _c) in self.items.items()
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            await store.open()
            await store.add(
                "Tim commutes on a gravel bicycle every day.", kind="fact", source_ref=SourceRef("user", "")
            )
