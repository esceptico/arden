...
def _hunks(base: tuple[bytes, ...], changed: tuple[bytes, ...]) -> tuple[_Hunk, ...]:
    ...
    return tuple(_Hunk(i1, i2, changed[j1:j2]) for tag, i1, i2, j1, j2 in matcher.get_opcodes() if tag != "equal")
...
def three_way_merge(base: bytes | None, current: bytes, generated: bytes) -> MergeResult:
    ...
    generated_hunks = tuple(hunk for hunk in _hunks(base_lines, generated_lines) if hunk not in current_hunk_set)
...
def synthesis_base_rel(path: str, revision: str) -> Path:
    ...
    return Path(".arden/maintenance/synthesis-bases") / synthesis_page_key(path) / f"{name}.md"


def synthesis_candidate_rel(path: str, revision: str) -> Path:
    name = revision or "empty"
    return Path(".arden/maintenance/synthesis-candidates") / synthesis_page_key(path) / f"{name}.md"
