...
_ENGINE_DIRS = {"raw", ".arden", ".index", ".maintenance"}
...
class VaultIndexer:
    ...
    def _children(
        ...
        for child in sorted(child_dirs, key=lambda p: (p.name.casefold(), p.name)):
            label = f"{child.name}/"
            entries.append(
                IndexEntry(label, self._description(self.root / child / "README.md", existing.get(label)), True)
            )
        for child in sorted(child_files, key=lambda p: (p.name.casefold(), p.name)):
            entries.append(
                IndexEntry(child.name, self._description(self.root / child, existing.get(child.name)), False)
            )
        return tuple(entries)
    ...
    def _existing_descriptions(self, directory: Path) -> dict[str, str]:
        ...
        if bounds is None:
            return {}
        _start, _end, content_start, content_end = bounds
        block = text[content_start:content_end]
        descriptions: dict[str, str] = {}
        for line in block.splitlines():
            stripped = line.strip()
            identity = PATH_RE.match(stripped)
            if identity:
    ...
    def _without_managed_block(text: str) -> str:
        ...
        if bounds is not None:
            start, end, _content_start, _content_end = bounds
    ...
    def _render_block(entries: tuple[IndexEntry, ...]) -> str:
        ...
        return "\n".join(
            f"- {entry.path} — {entry.description} <!-- arden:path={quote(entry.path, safe='')} -->"
            for entry in entries
        )

    @staticmethod
    def _marker_bounds(text: str) -> tuple[int, int, int, int] | None:
        matches: list[tuple[int, int, int, int]] = []
        partial = False
        for start_marker, end_marker in INDEX_MARKER_PAIRS:
            ...
            if len(starts) != 1 or len(ends) != 1 or starts[0] >= ends[0]:
                partial = True
                continue
            matches.append((starts[0], ends[0] + len(end_marker), starts[0] + len(start_marker), ends[0]))
    ...
    def _write_atomic(self, rel: Path, content: bytes) -> None:
        ...
        if target_st is not None and (stat.S_ISLNK(target_st.st_mode) or not stat.S_ISREG(target_st.st_mode)):
            raise FileNotFoundError(rel.as_posix())
        temp = path.parent / f".arden-{path.name}-{os.getpid()}-{uuid4().hex}.tmp"
        try:
