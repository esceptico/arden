...
from arden.memory.models import Kind, SourceRef, now_iso
...
async def _build_catalog(store) -> list:
    ...
    return [
        r
        for r in await store.list(limit=CATALOG_LIMIT * 2, scopes=None, kinds=_DURABLE_KINDS)
...
async def run_dream(
    ...
    *,
    ...
    questions = await _ask(
        ...
        model,
        reasoning_effort,
        _with_preamble(_QUESTIONS_SYSTEM, conventions, None),
        f"MEMORY CATALOG:\n{catalog}",
    )
    ...
    raw = await _ask(
        ...
        model,
        reasoning_effort,
        _with_preamble(_INSIGHTS_SYSTEM, conventions, learnings),
        "QUESTIONS:\n" + "\n".join(qs) + f"\n\nEVIDENCE:\n{evidence}",
    )
