...
def _validate_snapshot(snapshot: LinkIndexSnapshot) -> None:
    ...
    if len(snapshot.pages) != len(set(snapshot.pages)) or snapshot.pages != tuple(
        sorted(snapshot.pages, key=_path_key)
    ):
    ...
    for link in snapshot.links:
        ...
        if tuple(sorted(link.candidates, key=_path_key)) != link.candidates or len(set(link.candidates)) != len(
            link.candidates
        ):
