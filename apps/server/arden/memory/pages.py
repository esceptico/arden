...
class Page:
    ...
    def active_entries(self) -> tuple[LedgerEntry, ...]:
        ...
        return tuple(entry for entry in entries if entry.meta.operation == "record" and entry.id not in inactive)
...
def parse_raw(text: str) -> tuple[dict, list[Line | LedgerEntry]]:
    ...
    header_rows = [index for index, row in enumerate(rows) if RECORDS_V2_RE.fullmatch(row)]
    ...
    if first is not None and header_rows:
        ...
        while index < len(rows):
            if not rows[index].strip():
                index += 1
                continue
            if index + 1 >= len(rows) or not rows[index + 1].lstrip().startswith(META_PREFIXES):
...
def merge_split(page: Page, raw_text: str | None) -> Page:
    ...
    if raw_text:
        ...
        page.records_header = next(
            (row for row in raw_body.splitlines() if RECORDS_V2_RE.fullmatch(row)),
            None,
        )
