...
Use list_wiki_pages() for directory discovery, then read_wiki_page() and wiki_links() for relevant pages instead of loading the whole wiki. Read an existing directory's README.md before processing its pages; the wiki creates missing semantic README files atomically with the first page in a directory. For an explicit page request, use create_wiki_page(), edit_wiki_page(), move_wiki_page(), or archive_wiki_page() with the exact repository head and page version returned by a fresh read. move_wiki_page() changes only a path: it atomically updates resolved path-style links, never title-style links or redirects. Scheduled automations may create, edit, move, or archive wiki pages only below automations/. Use publish_wiki_generated() only when an explicit automation owns that page's generated region. Use fact tools when a claim needs verification or a stored correction. The wiki is the normal browsing surface; facts are the internal correctness layer. web_search is external information, while email/calendar/Slack are deferred data sources.
