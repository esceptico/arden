...
_TOOL_LOADING = (
    '**Tool loading** — Some integration/action tools are deferred. Use `load_tools` proactively when the user needs email, calendar, Slack, wiki browsing or publishing, automation, notification, directives, file write/edit, session management (creating/renaming/archiving chats, reading other sessions), fact history or corrections, skill creation, MCP-backed capabilities, or controls for a running agent. Load multiple known tools together with `load_tools(names=[...])`. Loading tools does not execute them; it only makes deferred tools callable on the next model step. Do not ask the user whether to load tools. Never use filesystem/time/no-op tool calls to discover or unlock deferred tools; call `load_tools(group="slack")` directly for Slack.',
    '**Tool loading** — Some integration/action tools are deferred. Use `tool_search(query="select:<tool_name>,<tool_name>")` proactively when the user needs email, calendar, Slack, wiki browsing or publishing, automation, notification, directives, file write/edit, session management (creating/renaming/archiving chats, reading other sessions), fact history or corrections, skill creation, MCP-backed capabilities, or controls for a running agent. Load multiple known tools in one selection. Loading tools does not execute them; it only makes deferred tools callable on the next model step. Do not ask the user whether to load tools. Never use filesystem/time/no-op tool calls to discover or unlock deferred tools.',
)
_DATA = (
