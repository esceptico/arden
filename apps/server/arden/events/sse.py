"""
Server-Sent Event types for Arden's chat stream.
...
remaining arden-specific events (approval, background tasks, automations,
...
class ToolCallArgsEvent(SSEEvent):
    ...
    tool_call_id: str = ""
    delta: str = ""  # JSON-encoded args (arden emits atomically, so single delta)
    depth: int = 0
    parent_id: str | None = None
...
# ─── arden-specific (non-canonical) events ─────────────────────────────
...
def event_from_payload(payload: dict) -> SSEEvent:
    ...
    kwargs = {
        key: value for key, value in payload.items() if key not in {"type", "seq", "session_id", "workflow_state"}
    }
    return cls(**kwargs)
...
def agent_events_to_sse(event) -> tuple[SSEEvent, ...]:
    """Convert an arden.agent event to one or more SSEEvents.

    For tool calls we emit the full AG-UI start/args/end triplet; arden
