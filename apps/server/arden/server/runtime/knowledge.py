    ...
    async def _run(self) -> None:
        ...
        try:
            ...
            if failed:
                ...
                self.stale = True
                if not self._closed:
                    self._retry_handle = asyncio.get_running_loop().call_later(self._retry_delay, self.schedule, ())
                return
            self.stale = False
        finally:
            self._task = None
...
class KnowledgeRuntime:
    ...
    def start_memory_watch(self, on_change) -> None:
        ...
        if store is not None and hasattr(store, "start_watch"):
            ...
            async def _indexed_change(changes):
                from arden.memory.file_store import ObservedFileChange
    ...
    async def _init_memory(self, stores: Stores) -> None:
        ...
        from arden.memory.project_names import load_project_names
        ...
        from arden.memory.scorer import score_importance
        ...
        from arden.memory.daily import DailyProjector
